...
(transcript, turns, interruption events, STT confidence) — not a stub. What's
explicitly NOT built in this pass: real-LLM-generated prose summaries (the
mock summary is structured-fact-based, not natural language) and revenue
attribution (no real payment/CRM data exists yet to attribute from — see
docs/IMPLEMENTATION_CHECKLIST.md Phase 4 notes).
"""
...
# ~12s of speech at services/voice-worker/app/turn_manager.py's ~55ms/char
# simulated speaking rate (220 * 55ms ≈ 12.1s) — deliberately a copied
# derived constant, not a cross-service import (services stay independently
# deployable, spec §36); if the two ever need to match to the millisecond,
# that's a sign to promote the rate itself into packages/db.
...
async def _dispatch_follow_up(db, *, workspace_id, follow_up_task: FollowUpTask, category: str) -> None:
    """Immediately actions a just-created FollowUpTask via the tool
    framework, rather than leaving it "pending" forever — this pass has no
    real task scheduler, so "follow up later" is realized as "follow up now"
    (documented simplification, docs/IMPLEMENTATION_CHECKLIST.md Phase 6).
    `reminder`/`close` genuinely have nothing to do yet (no scheduler /
    no action by definition) and are left pending/completed accordingly."""
    ...
    if channel == "whatsapp":
        ...
        try:
            ...
            pass  # tool not configured for this workspace — leave pending for a human to action
        ...
        # Not one of the ten business tools (spec §17's catalog) — suppression
        # is a contacts-domain action, not something an external system does.
...
async def run_post_call_pipeline(db: AsyncSession, *, workspace_id: uuid.UUID, call_id: uuid.UUID, encryption_key: str) -> dict:
    ...
    # Real trigger for the generic webhook integration (docs/IMPLEMENTATION_CHECKLIST.md
    # Phase 9) — fires once per completed call, after the outcome/summary/
    # quality rows above are already committed-in-flight, so a subscriber
    # that immediately calls back into the API sees consistent data.
...
async def _upsert_outcome(db, *, workspace_id, call_id, category, lead_score, reasons, objective_status) -> None:
    ...
    if row is None:
        db.add(
            CallOutcome(
                ...
                score_reasons=reasons, objective_status=objective_status, notes="Generated by intelligence-worker's post-call pipeline.",
            )
        )
    else:
        ...
        row.notes = "Generated by intelligence-worker's post-call pipeline."
...
async def _upsert_summary(db, *, workspace_id, call_id, summary_text) -> None:
...
def _evaluate_quality(*, agent_turns, customer_turns, meaningful_interruptions, total_interruptions, objective_status, knowledge_events, transcript_text) -> dict:
    ...
    hallucination_flag = False  # mock LLM only ever says scripted or knowledge-grounded text
...
async def _upsert_quality(db, *, workspace_id, call_id, quality: dict) -> None:
