...
export default async function AgentVoicePage({
  params,
  searchParams,
}: {
  ...
  if (!versionId) return <EmptyState title="No version yet" description="This agent has no versions." />;

  const version = await agentsApi.getVersion(workspace.id, agent.id, versionId, { cookieHeader });
  return <VoiceEditor workspaceId={workspace.id} agentId={agent.id} version={version} />;
}
